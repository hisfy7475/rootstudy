...
// 멘토링/클리닉/상담 유형 라벨 (학생 캘린더, 어드민 그리드, 알림 공통)
export const MENTORING_TYPE_LABEL = {
  mentoring: '멘토링',
  clinic: '클리닉',
  consult: '상담',
} as const;
...
export const FOCUS_SCORE_TEMPLATES: Record<number, FocusScoreTemplate> = {
  10: {
    label: '최상 몰입 단계',
    short:
      '학습 시간 전반에 걸쳐 외부 자극에 흔들림 없이 높은 집중력을 유지하였으며, 계획된 학습 과제를 스스로 점검하고 완성도 높게 수행하였습니다.\n학습 태도, 지속력, 자기조절 능력이 모두 우수하여 자기주도 학습 역량이 안정적으로 자리 잡은 상태입니다.',
  ...
  7: {
    label: '집중 관리 단계',
    short:
      '학습 중 주변 환경이나 피로 등의 영향으로 집중력이 일시적으로 저하되는 모습이 있었으나, 지도 후 점차 학습 흐름을 회복하였습니다.\n장시간 집중 유지에는 다소 보완이 필요하여 학습 밀도 향상을 위한 지속적인 관리가 이루어지고 있습니다.',
...
export const COUNSELING_TEMPLATES = {
  ...
  getGuidanceNotes(focusAvg: number | null): string {
    ...
    if (focusAvg >= 9)
      return '현재의 학습 흐름을 유지하면서 심화 학습에 도전해보는 것을 권장합니다.';
    if (focusAvg >= 8)
      return '최상위 몰입 단계로의 도약을 위해 학습 밀도를 높이는 관리를 병행하고 있습니다.';
  ...
  buildParentSummaryWithFeedback(
    ...
    const focusText = focusAvg !== null ? `평균 ${Math.round(focusAvg * 10) / 10}점` : '미측정';
