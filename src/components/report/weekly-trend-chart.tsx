...
export function WeeklyTrendChart({ data }: WeeklyTrendChartProps) {
    ...
    return (
      <Card>
        <CardHeader className='pb-2'>
          <h3 className='text-text text-lg font-semibold'>주별 학습현황</h3>
        </CardHeader>
        <CardContent>
          <p className='text-text-muted py-6 text-center text-sm'>표시할 데이터가 없습니다.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const maxSeconds = Math.max(
    ...data.flatMap((d) => [d.mySeconds, d.gradeMaxSeconds, d.gradePeerAvgSeconds]),
    3600,
  );
  ...
  const getY = (seconds: number) => padT + chartH - (seconds / maxSeconds) * chartH;
  ...
  const poly = (vals: number[]) => vals.map((v, i) => `${getX(i)},${getY(v)}`).join(' ');

  const myPts = data.map((d) => d.mySeconds);
  const maxPts = data.map((d) => d.gradeMaxSeconds);
  const avgPts = data.map((d) => d.gradePeerAvgSeconds);
    ...
    <Card>
      <CardHeader className='pb-2'>
        <div className='flex items-start justify-between gap-2'>
          <h3 className='text-text text-lg font-semibold'>주별 학습현황</h3>
          <div className='text-text-muted flex flex-col items-end gap-1 text-[9px] leading-tight'>
      ...
      <CardContent>
        ...
        >
          {gridLines.map((line, i) => (
            <g key={i}>
              <line
                ...
                strokeWidth='1'
              />
              <text x={padL - 6} y={line.y + 3} textAnchor='end' fontSize='8' fill='#9ca3af'>
                {line.label}
              </text>
            </g>
          ))}
          ...
          {data.map((d, i) => (
            <text key={i} x={getX(i)} y={vbH - 8} textAnchor='middle' fontSize='9' fill='#6b7280'>
              {d.weekLabel}
            </text>
          ))}
        </svg>
