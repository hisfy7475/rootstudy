...
export function MockExamOrdersClient({ initialOrders }: { initialOrders: MealOrderWithProduct[] }) {
  ...
  return (
    <ul className='space-y-3'>
      {orders.map((o) => {
        ...
        const name = p?.name ?? "모의고사";
        const examStart = p?.meal_start_date;
        const canCancel =
          o.status === "paid" && examStart && canCancelMealOrderByDeadline(examStart);
