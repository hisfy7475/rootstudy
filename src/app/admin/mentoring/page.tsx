...
export default async function AdminMentoringWeekPage({ searchParams }: PageProps) {
  ...
  const monday = sp.w && /^\d{4}-\d{2}-\d{2}$/.test(sp.w) ? sp.w : getMondayOfWeekKST(today);
  ...
  return (
    <div className='mx-auto max-w-[1400px] space-y-6 p-4 md:p-8'>
      <div className='flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between'>
        <div>
          <h1 className='text-2xl font-bold tracking-tight'>멘토링/클리닉/상담 주간 일정</h1>
        ...
        <div className='flex flex-wrap items-center gap-2'>
          ...
          <Link
            ...
            className={cn(
              'bg-primary text-primary-foreground inline-flex items-center gap-1 rounded-2xl px-4 py-2 text-sm font-medium',
              'hover:bg-primary/90',
            )}
          >
