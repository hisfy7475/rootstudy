...
interface AdminMockExamOrdersClientProps {
  product: MealProduct;
  initialOrders: MealOrderForAdmin[];
}

export function AdminMockExamOrdersClient({
  product,
  initialOrders,
}: AdminMockExamOrdersClientProps) {
  ...
  const exportXlsx = async () => {
    ...
    try {
      const rows = orders.map((o) => ({
        ...
        시험시작일: product.meal_start_date,
        시험종료일: product.meal_end_date,
      }));
      ...
      XLSX.writeFile(wb, `모의고사신청_${product.name}_${getTodayKST()}.xlsx`);
    } finally {
      setExporting(false);
    }
  };
    ...
    <div className='space-y-6'>
      <div className='flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between'>
        ...
        <div className='flex flex-wrap gap-2'>
          <Link
            href={`/admin/mock-exams/${product.id}`}
