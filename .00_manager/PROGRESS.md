...
| 2026-07-27 | 멘토링·상담 참여 자동 상점: `mentoring_reward_grants` 원장(application_id PK) + 지점별 `reward_presets(code='mentoring_attend')` 시드 + `uq_points_study_date_preset`에서 `auto_mentoring` 제외(세션당 1점), `/api/cron/mentoring-reward`(KST 09:00, 확정 신청 1건=1점, 7일 소급, dryRun/from/to/notify), 지난 슬롯 대리등록 정원 해제(`ensureCapacityForPastSlot`)·지난 슬롯 강제취소 알림 억제, `deleteRewardPreset` `is_system` 가드. **dev 적용·검증 완료, prod 미적용. 배포 전 `MENTORING_REWARD.startDate`(현재 2026-07-28)를 실제 배포일 다음날로 재확인 필요.** |
