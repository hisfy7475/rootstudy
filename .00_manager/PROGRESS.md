...
| 2026-07-27 | 멘토링·상담 참여 자동 상점: `mentoring_reward_grants` 원장(application_id PK) + 지점별 `reward_presets(code='mentoring_attend')` 시드 + `uq_points_study_date_preset`에서 `auto_mentoring` 제외(세션당 1점), `/api/cron/mentoring-reward`(KST 09:00, 확정 신청 1건=1점, 7일 소급, dryRun/from/to/notify), 지난 슬롯 대리등록 정원 해제(`ensureCapacityForPastSlot`)·지난 슬롯 강제취소 알림 억제, `deleteRewardPreset` `is_system` 가드. **dev 적용·검증 완료. prod 마이그레이션도 적용 완료(`schema_migrations` `20260727022014/022026/022035` 확인, 전 지점 `mentoring_attend` 프리셋 시드됨). 배포 전 `MENTORING_REWARD.startDate`(현재 2026-07-28)를 실제 배포일 다음날로 재확인 필요.** |
| 2026-07-27 | 급식 문의 2건 대응 — ① **통합신청내역 취소일 표시**(유지): `unified_applications` 뷰의 `meta.cancelled_at`을 표(상태 배지 아래)·모바일 카드·엑셀(`결제일` 옆 `취소일`)에 노출. 뷰에 이미 3개 도메인 모두 값이 있어 마이그레이션 불필요(`applications-client.tsx` 단일 파일). ② **비활성 상품 셀프 환불 차단**(`5edd819`) → **롤백**(코드 원복). 상품 `status='inactive'`면 학생·학부모 셀프 환불을 막고 관리자 환불만 허용하는 구현이었고, 운영이 판매 마감일에 상품을 비활성으로 내리는 관행(비활성 68개 중 67개가 판매 종료 후 전환) 때문에 소급 적용 시 이용 전 결제 14건이 막혀 `INACTIVE_REFUND_BLOCK_EFFECTIVE_AT`(2026-07-27)로 시행일 이후 결제분에만 적용(`42801ae`). 배포 후 클라이언트가 **보류** 요청 — "압구정은 비활성 시 환불 중지, 반포는 이틀 전 환불 가능"으로 **지점별 차등**이 필요한데 구현이 전 지점 일괄이었음. `2ce15b0` 시점 파일 복원으로 4개 파일(`meal-order-rules.ts`, `meal-payment-cancel.ts`, `actions/meal.ts`, `user-orders-client.tsx`) 원복, 취소일 기능은 유지. **재도입 시 주의: `INACTIVE_REFUND_BLOCK_EFFECTIVE_AT`를 그대로 부활시키면 롤백 기간 결제분이 전부 소급 차단되므로 재도입 배포일 기준으로 재산정할 것.** 지점별 정책은 `meal_products.branch_id`가 이미 있어 `canCancelOrder` 게이팅으로 가능하나, 관리자 토글을 두려면 `branches` 컬럼 추가 + 지점 관리 화면 작업 필요(반포는 기본 규칙 유지 = 현행 그대로, 압구정만 ON). 참고: 소급판이 11:16~11:29 약 13분간 운영에 노출(차단은 PG 호출·`payment_logs` 기록 전에 리턴하므로 DB 흔적 없음), 12:19에 비활성 상품 3건(33,000원)이 정상 셀프 환불됨. |
| 2026-07-27 | 일일 자동 상점(`일일 학습 3시간 + 과목 분류`, `event_kind='auto_daily_focus'`) 부여 중단 — 클라이언트 요청. `REWARD_RULES.dailyFocusEndDate='2026-08-01'` + `isDailyFocusActive()`/`isDailyFocusHistoryVisible()`(`src/lib/utils.ts`) 게이트로 `evaluateDailyFocus` 조기 리턴(`skipped:'daily_focus_ended'`), 진행도 위젯은 즉시·최근 7일 스트립은 종료 +7일(8/8)까지 노출. `daily_focus` 프리셋 `is_active=false` + seed 함수도 비활성 시드로 변경(줄 삭제 시 롤백이 조용히 깨짐). 정책 페이지 ⑥을 '상점 획득 경로'로 교체(+상품권 자동 등록 오기 정정). **과거 부여분 917건 미회수**(DELETE·`cancel_point` 모두 차단, 상품권 교환 기발생). 상점 적립 약 22% 감소 — 현 벌점 20점 이상 4명은 이 항목 수령분이 0이라 상계 여력 즉시 영향 없음. **dev·prod 모두 적용 완료(2026-07-27). 코드는 PR #47 병합 후 Vercel 배포 완료, prod 마이그레이션 적용·검증 완료(전 지점 `daily_focus` 비활성·`mentoring_attend` 유지).** 후속: ① 8월 중순 `vercel.json` `daily-focus-notify` 크론 제거(7/31분 알림 때문에 지금은 유지) ② 위젯·스트립·`getTodayFocusProgress` 데드코드 정리 ③ 분류 유인 소멸에 따른 리포트 '미분류' 비중 추이 관찰 ④ `POLICY_VERSION` 재고지 경로는 미배선 상태(`checkPolicyAcknowledgement` 호출자 0개, 학부모 진입점 없음)라 공지사항으로 안내 |
| 2026-07-27 | **비활성 상품 셀프 환불 차단 재적용** — 같은 날 롤백(`5fbc1ab`)을 되돌려 클라이언트 요청대로 전 지점 일괄 재적용. `5271d53` 시점 파일 복원으로 4개 파일이 `42801ae`(소급 제거 포함) 상태로 정확히 복귀(롤백 커밋의 역방향, 그 사이 해당 파일을 건드린 커밋 없음). **`INACTIVE_REFUND_BLOCK_EFFECTIVE_AT`(2026-07-27 00:00 KST)는 재산정 불필요** — 롤백 기간이 당일 몇 시간뿐이고, 시행일 이후 결제 21건이 전부 활성 상품이라 재배포 즉시 차단 대상 0건(prod 실측). 지점별 차등을 넣지 않은 근거: 상품을 **환불 기한 전에** 비활성으로 내린 비율이 압구정 39/67(58%)인 반면 반포는 1/37(3%, 그마저 결제 0건)이라, 반포는 기한 판정이 먼저 걸려 기존 안내 문구가 그대로 뜨고 체감 환불 기간이 바뀌지 않음. **단 이는 운영 관행에 기댄 것이지 코드 보장이 아니며, 반포가 조기 마감하면 차단됨** — 마찰 발생 시 `meal_products.branch_id` 기반 게이팅 + `branches` 토글 컬럼(마이그레이션) + 지점 관리 화면으로 분기할 것. dev 검증: 시행일 이후 결제 + 비활성 → 취소 버튼 숨김 + "판매가 중지된 상품입니다" 노출, 시행일 이전 결제 + 비활성 → 취소 버튼 유지(소급 없음 확인). 통합신청내역 취소일 표시는 롤백·재적용 양쪽 모두 무영향(`5edd819`와 byte-identical). |
