...
let WQIsiPhoneX: Bool = UIScreen.instancesRespond(to: #selector(getter: UIScreen.currentMode)) &&
       (UIScreen.main.currentMode!.size == CGSize(width: 1125, height: 2436) ||
        UIScreen.main.currentMode!.size == CGSize(width: 1242, height: 2688) ||
        UIScreen.main.currentMode!.size == CGSize(width: 828, height: 1792))
...
public class WQPhotoBaseViewController: UIViewController {
    ...
    lazy var rightButton: UIButton = {
        ...
        rightButton.frame = CGRect(x: WQScreenWidth-60, y: WQStatusBarHeight, width: 60, height: WQNavigationHeight)
    ...
    func setRightImageButton(normalImage: UIImage?, selectedImage: UIImage?, isSelected: Bool) {
        rightButton.imageEdgeInsets = UIEdgeInsets(top: 10, left: 18, bottom: 10, right: 18)
