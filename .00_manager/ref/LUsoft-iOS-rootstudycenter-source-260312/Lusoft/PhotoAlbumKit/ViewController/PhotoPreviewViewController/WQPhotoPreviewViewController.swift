...
class WQPhotoPreviewViewController: WQPhotoBaseViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    private func initNavigation() {
        self.setBackNav()
        if let index = self.photoData.assetArray.firstIndex(of: self.previewPhotoArray[currentIndex]) {
...
//        self.bottomView.backgroundColor = UIColor(white: 0.1, alpha: 0.9)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if let index = self.photoData.assetArray.firstIndex(of: self.previewPhotoArray[self.currentIndex]) {
